...
app.get('/api/currentuser', auth.isAuthenticated, getMe);
app.get('/api/users/:id', getUser);
app.put('/api/users/:id', auth.isInRole(auth.role.ADMIN), upsertUser);
app.delete('/api/users/:id', auth.isInRole(auth.role.ADMIN), disableUser);

// listUsers lists all users in the system
function listUsers (req, res) {
  promiseResponse(userStore.find({ disabled: { $ne: true } }), res);
}
